...
namespace Graphics2D
{
    ...
    {
        private Draw2DBase jigged = null;

        protected Editor Editor { get; private set; }
        protected TOptions Options { get; private set; }
        protected Draw2DBase Jigged
        {
            get { return jigged; }
            set
            {
        ...
        private void Editor_CursorMove(object sender, CursorEventArgs e)
        {
            ...
            if (Editor.Document.Settings.Snap)
            {
                ...
                foreach (Draw2DBase item in Editor.Document.Model)
                {
                    if (item.Visible && (item.Layer == null || item.Layer.Visible))
                        Editor.SnapPoints.AddFromDraw2DBase(item, e.Location, snapMode, snapDist);
                }
            }
